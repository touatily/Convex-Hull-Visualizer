...
class Window(tk.Tk):
    def __init__(self):
        ...
        list_algos = ["Gift wrapping", "Graham scan",
                      "Quickhull", "Monotone chain"]
        self.list_algos = ttk.Combobox(self.plan2, values=list_algos,
                                       state="readonly")
        self.list_algos.pack(ipadx=15, padx=10, ipady=5, pady=5)

        self.list_algos.bind("<<ComboboxSelected>>", self.selectAlgo)

        self.speed = tk.Scale(self.plan2, from_=1, to=100,
                              orient=tk.HORIZONTAL, width=10,
                              label="Speed", length=100)
        self.speed.pack(ipadx=15, padx=10)

        self.simulateBtn = tk.Button(self.plan2, text="Simulate",
        ...
        self.complexity.set("-")
        self.complexity_val = tk.Label(self.plan2,
                                       textvariable=self.complexity,
        ...
        self.description_val = tk.Label(self.plan2,
    ...
    def movePoint(self, event=None):
        if self.pickedPoint:
            ...
            self.canvas.itemconfig(self.pickedPoint,
                                   tag=f"point {event.x}_{event.y}")

    def pickPoint(self, event=None):
        x, y = event.x, event.y
        points = [(self.canvas.coords(i), i) for i in
                  self.canvas.find_withtag("point")]
        if points:
            ll = [(math.sqrt((e[0][0]+3-x)**2 + (e[0][1]+3-y)**2), e[1])
                  for e in points]
    ...
    def selectAlgo(self, event=None):
        if self.list_algos.current() == -1:
            self.complexity.set("-")
            self.description.set("-")
        elif self.list_algos.current() > 7:
            return
        else:
            self.complexity.set(self.algos_details[self.list_algos.current()]
                                                  ["complexity"])
            self.description.set(self.algos_details[self.list_algos.current()]
                                                   ["description"])
